...
class Myserver(socketserver.BaseRequestHandler):
	...
	def my_recv(self,protocol = False,msg_len = 1024):
		...
		return opt_dic
	# 由于用到了quest方法，因此只能写在Myserver类中

	def upload(self,opt_dic,base_name):
	...
	def download(self,opt_dic,base_name):
		remote_path = os.path.join(base_path, base_name)
		...
		if os.path.isfile(file):
			...
			logging.info('%s用户下载了%s文件' % (base_name,opt_dic['file']))
		else:
			dic = {'isfile':'no'}
			self.my_send(dic,True)
			logging.warning('没有%s文件' % opt_dic['file'])

	def quit(self,opt_dic,base_name):
		dic = {'signal':'再见!','operate':opt_dic['operate']}
		self.my_send(dic,True)
			...
			while True:
				...
				if dic['flag']:
					while True:
						...
						if hasattr(Myserver,opt_dic['operate']):
							func = getattr(Myserver,opt_dic['operate'])
							func(self,opt_dic,base_name)
		except Exception:
...
sk = socketserver.ThreadingTCPServer(('127.0.0.1',9005),Myserver)
